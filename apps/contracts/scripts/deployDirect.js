...
async function main() {
  ...
  const AxCNHFactory = await ethers.getContractFactory("AxCNH");
  const AxPesaVaultFactory = await ethers.getContractFactory("AxPesaVault");

  console.log("\n📝 Deploying AxCNH Token...");
  const axcnh = await AxCNHFactory.connect(wallet).deploy(wallet.address);
  ...
  const vaultAxcnh = await AxPesaVaultFactory.connect(wallet).deploy(
    axcnhAddress,
    signers,
    wallet.address,
    "AxCNH"
  ...
  for (const token of existingTokens) {
    ...
    const vault = await AxPesaVaultFactory.connect(wallet).deploy(
      token.address,
      signers,
      wallet.address,
      token.name
